#include <cstddef>
#include "NonDominatedSorting.h"

void CNonDominatedSorting::Cluster(std::vector<CIndividual*>& population, std::vector<std::vector<size_t>>& clusters)
{
size_t popSize = population.size();
	...
	for (size_t c = 0; c < clusters.size(); ++c)
	{
		...
		{
			population[clusters[c][i]]->SetRank(c);
		}
	}
}
