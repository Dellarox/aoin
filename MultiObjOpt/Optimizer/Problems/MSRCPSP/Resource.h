...
public:
	...
	TResourceID GetResourceID() const { return m_ID; }

	TTime GetFinish() const { return m_Finish; }
	void SetFinish(TTime finish) { m_Finish = finish; }

	TTime GetWorkingTime() const { return m_WorkingTime; }
	void SetWorkingTime(TTime workingTime) { m_WorkingTime = workingTime; }
	void AddWorkingTime(TTime workingTime) { m_WorkingTime += workingTime; }

	float GetSalary() const { return m_Salary; }

	const std::vector<SSkill>& GetSkills() const { return m_Skills; }
