...
void CTTP::SetData(const std::vector<SCity>& cities, const std::vector<SItem>& items, int capacity, float minSpeed, float maxSpeed, float rentingRatio)
{
	Clear();
...
float CTTP::GetMaxTravelTime() const
{
	return CalculateMaxDistance() / m_MinSpeed;
}

float CTTP::GetMaxProfit() const
{
...
float CTTP::GetMinTravelTime() const
{
	return CalculateMinDistance() / m_MaxSpeed;
}

float CTTP::GetMinProfit() const
{
	return 0.f;
}

float CTTP::CalculateMaxDistance() const
{
	// Based on Java impl
	return CalculateMinDistance() * 2.f;
}

float CTTP::CalculateMinDistance() const
{
	float dist = 0.f;
...
void CTTP::CalculateContextData()
{
