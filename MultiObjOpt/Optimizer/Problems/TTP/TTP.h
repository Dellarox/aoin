...
#define USE_EOK 0
// TODO - KNAP_ORDER_ENCODING
// TODO - SAVE_FIXED_GENES 1
...
public:
	...
	void SetData(const std::vector<SCity>& cities, const std::vector<SItem>& items, int capacity, float minSpeed, float maxSpeed, float rentingRatio);

	const std::vector<SCity>& GetCities() const { return m_Cities; }
	const std::vector<SItem>& GetItems() const { return m_Items; }
	...
	const std::vector<std::vector<size_t>>& GetCityItems() const { return m_CityItems; }
	const std::vector<size_t>& GetProfitRatioSortedItems() const { return m_ProfitRatioSortedItems; }

	int GetCapacity() const { return m_Capacity; }
	float GetMinSpeed() const { return m_MinSpeed; }
	float GetMaxSpeed() const { return m_MaxSpeed; }
	float GetRentingRatio() const { return m_RentingRatio; }

	size_t GetCitiesSize() const { return m_Cities.size(); }
	size_t GetItemsSize() const { return m_Items.size(); }
